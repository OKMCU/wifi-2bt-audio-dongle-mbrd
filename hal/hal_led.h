 ...
 * Set the LED ON/OFF.
 */
extern void hal_led_set( uint8_t leds, uint8_t mode );

/*
 * Blink the LED.
 */
extern void hal_led_blink( uint8_t leds, uint8_t cnt, uint8_t duty, uint16_t time );

/*
 * Put LEDs in sleep state - store current values
 */
extern void hal_led_enter_sleep( void );

/*
 * Retore LEDs from sleep state
 */
extern void hal_led_exit_sleep( void );
